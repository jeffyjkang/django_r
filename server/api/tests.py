...
class CachingTestCase(APITestCase):
  def test_wishlist_cache(self):
    ...
    res = self.client.post('/api/v1/wishlist/', { 'id': package.id })
    self.assertIsNone(cache.get('wishlist:wishlist-items'))
    res = self.client.get('/api/v1/wishlist/')
    self.assertListEqual(res.data, [package.id])
...
class SortingFilteringTestCase(APITestCase):
  ...
  def test_sorting_and_filtering(self):
    ...
    res = self.client.get('/api/v1/public/packages/', **auth_header(token))
    ids = list(map(lambda res: res['id'], res.data['results']))
    self.assertListEqual(ids, [expensive_package.id, discount_package.id])
    res = self.client.get('/api/v1/public/packages/?search=a', **auth_header(token))
    ids = list(map(lambda res: res['id'],res.data['results']))
    self.assertListEqual(ids,[discount_package.id])
    res = self.client.get('/api/v1/public/packages/?price_min=50.00', **auth_header(token))
    ids = list(map(lambda res: res['id'], res.data['results']))
...
class ValidationTestCase(APITestCase):
  def test_invalid_street_address_returns_error(self):
    ...
    res = self.client.post('/api/v1/bookings/', data)
    self.assertEqual(res.status_code, 400)
    self.assertEqual(
      res.data['street_address'][0],
